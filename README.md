...
-  **Interactive Q&A Interface** - Walks through building universe specifications
-  **Bottom-Up Approach** - Builds the universe stack from target to global universe
-  **Tally Generation** - Creates F-card specifications with `<` operator syntax
-  **SDEF Generation** - Uses distribution method (SI/SP cards)
-  **Lattice Support** - Handles LAT=1 and LAT=2 lattices with index notation
